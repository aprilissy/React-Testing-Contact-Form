...
test('user can fill out and submit form',async () => {
  ...
  fireEvent.change(firstNameInput, { target:{ value:'Ap', name:'firstName'}});
  fireEvent.change(lastNameInput, { target:{ value:'Darger', name:'lastName'}});
  fireEvent.change(emailInput, { target:{ value:'a@a.com', name:'email'}});
  ...
  await waitFor(() => {    
    const firstNameText = screen.getByText(/ap/i)
    expect(firstNameText).toBeTruthy()
    const lastNameInput = screen.getByText(/darger/i)
    expect(lastNameInput).toBeTruthy()
    const emailInput = screen.getByText(/a@a.com/i)
